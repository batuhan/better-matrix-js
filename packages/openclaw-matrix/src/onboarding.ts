...
async function runMatrixConfigure(params: {
  ...
  if (!hasConfiguredSecretInput(accessToken) && !hasConfiguredSecretInput(password)) {
    const authMode = await params.prompter.select({
      ...
      options: [
        { value: "beeper", label: "Create or sign in to a Beeper account" },
      ...
      const email =
        normalizeStringifiedOptionalString(
          await params.prompter.text({
            ...
            validate: (value) => (normalizeOptionalString(value)?.includes("@") ? undefined : "Email required"),
          }),
        ) ?? "";
      const { createBeeperLogin } = await import("@beeper/pickle/beeper/auth");
      const account = await createBeeperLogin({
        email,
        initialDeviceDisplayName: "OpenClaw Gateway",
        getLoginCode: async () =>
          normalizeStringifiedOptionalString(
            await params.prompter.text({
